...
int copy_key(int key1, int key2){
    ...
    FILE* fptr2 = fopen(path2, "w+"); // If key2 doesnt exist, the file is created.
    
    // Verify for case in which key1 doesnt exist.
    if(fptr1 == NULL){
	    perror("Error: File with first key doesn't exist.\n"); 
	    printf("----------------------------------------\n");
    ...
    if(fptr2 == NULL){
	    perror("Error while opening or creating file with second key.\n");
	    printf("----------------------------------------\n");

	    return -1;
    }

    // Retrieve information for the remaining cases.
    if((fgets(line, MAX_MSG_SIZE, fptr1)) == NULL){
	    perror("Error while reading file with first key.\n");
    	    printf("----------------------------------------\n");

	    return -1;
    }

    size_t bytes_written = fwrite(line, sizeof(char), strlen(line), fptr2);
    
    if(bytes_written != strlen(line)){
	    perror("Error while writing to file with second key.\n"); 
    	    printf("----------------------------------------\n");
    ...
    // Close the file.
    if (fclose(fptr2) == EOF) {
    	    perror("Error while closing the file with second key.\n");
	    printf("----------------------------------------\n");
    ...
    if (fclose(fptr1) == EOF) {
    	    perror("Error while closing the file with first key.\n");
    	    printf("----------------------------------------\n");

  	    return -1;
    }

    printf("Tuple  %s has been copied to tuple  %s\n", key1_str, key2_str);
    printf("----------------------------------------\n");
