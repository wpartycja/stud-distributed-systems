...
static void
project_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
	...
	if (!project_1_freeresult (transp, _xdr_result, (caddr_t) &result))
...
main (int argc, char **argv)
{
	...
	pmap_unset (PROJECT, PROJECT_VER);
	...
	if (!svc_register(transp, PROJECT, PROJECT_VER, project_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (PROJECT, PROJECT_VER, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, PROJECT, PROJECT_VER, project_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (PROJECT, PROJECT_VER, tcp).");
		exit(1);
	}
