...
int
_copy_key_1 (copy_key_1_argument *argp, void *result, struct svc_req *rqstp)
{
	return (copy_key_1_svc(argp->key1, argp->key2, result, rqstp));
}

static void
project_rpc_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	union {
		...
		int exist_1_arg;
		copy_key_1_argument copy_key_1_arg;
	} argument;
	union {
		int init_1_res;
		int set_value_1_res;
	...
	case COPY_KEY:
		_xdr_argument = (xdrproc_t) xdr_copy_key_1_argument;
		_xdr_result = (xdrproc_t) xdr_int;
	...
	if (!project_rpc_1_freeresult (transp, _xdr_result, (caddr_t) &result))
...
main (int argc, char **argv)
{
	...
	pmap_unset (PROJECT_RPC, PROJECT_VER);
	...
	if (!svc_register(transp, PROJECT_RPC, PROJECT_VER, project_rpc_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (PROJECT_RPC, PROJECT_VER, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, PROJECT_RPC, PROJECT_VER, project_rpc_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (PROJECT_RPC, PROJECT_VER, tcp).");
		exit(1);
	}
